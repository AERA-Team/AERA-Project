...
//! - Support for Ed25519 (AERA), secp256k1 (ETH/TRON)
...
pub enum KeyAlgorithm {
    ...
    Ed25519,
    /// secp256k1 for Ethereum and TRON
    Secp256k1,
}
...
impl KeyVault {
    ...
    pub fn create_mnemonic_wallet(&mut self, password: &str) -> Result<(String, String), KeystoreError> {
        let mnemonic = Bip39Mnemonic::generate_in(Language::English, 12)
            .map_err(|e| KeystoreError::Encryption(e.to_string()))?;
...
mod tests {
    ...
    fn test_vault_mnemonic_mapping() {
        ...
        let eth1 = vault.get_address_by_purpose(KeyPurpose::Ethereum).unwrap();
        ...
        let eth1_again = vault.get_address_by_purpose(KeyPurpose::Ethereum).unwrap();
        assert_eq!(eth1, eth1_again);
        
        // Unlock wallet 2
        vault.unlock_session(password, Some(&addr2)).unwrap();
        let eth2 = vault.get_address_by_purpose(KeyPurpose::Ethereum).unwrap();
        
        assert_ne!(eth1, eth2, "Different wallets should have different ETH addresses");
    }
}
